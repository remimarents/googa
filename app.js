...
function showWelcome(){stopAudio();group=null;index=0;$('welcome').classList.remove('hidden');$('game').classList.add('hidden');document.body.removeAttribute('data-age');}
function chooseGroup(id){group=groups.find(g=>g.id===id);index=0;$('welcome').classList.add('hidden');$('game').classList.remove('hidden');document.body.dataset.age=id;renderRiddle();playPrompt(riddles[id][0],{auto:true});}
