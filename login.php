...
<main class="landing">
  ...
  <p class="lead" data-so="Ku baro Af-Soomaaliga halxiraalo, cod iyo ciyaar qoyska oo dhan ah." data-no="Lær somali gjennom gåter, lyd og lek for hele familien.">Ku baro Af-Soomaaliga halxiraalo, cod iyo ciyaar qoyska oo dhan ah.</p>
  <div class="plans">
    <form method="post" action="checkout.php"><input type="hidden" name="csrf" value="<?= htmlspecialchars((string)$_SESSION['googa_payment_csrf'], ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="kind" value="trial"><button class="plan" type="submit"><span data-so="Marka hore tijaabi" data-no="Prøv først">Marka hore tijaabi</span><b>kr 5</b><small data-so="2 maalmood, ka dib kr 50/bishii" data-no="2 dager, deretter kr 50/mnd">2 maalmood, ka dib kr 50/bishii</small><em data-so="Dooro →" data-no="Velg →">Dooro →</em></button></form>
    <form method="post" action="checkout.php"><input type="hidden" name="csrf" value="<?= htmlspecialchars((string)$_SESSION['googa_payment_csrf'], ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="kind" value="monthly"><button class="plan" type="submit"><span data-so="Rukhsad bille ah" data-no="Månedsabonnement">Rukhsad bille ah</span><b>kr 50</b><small data-so="bishii" data-no="per måned">bishii</small><em data-so="Dooro →" data-no="Velg →">Dooro →</em></button></form>
  </div>
  <div class="divider" data-so="Hore ayaad xisaab u leedahay?" data-no="Har du allerede en konto?">Hore ayaad xisaab u leedahay?</div>
  ...
  <p class="reset-row"><button class="reset-button" id="showReset" type="button" data-so="Ma illowday furaha? Samee mid cusub" data-no="Glemt passord? Opprett et nytt">Ma illowday furaha? Samee mid cusub</button></p>
  ...
  <?php if ($notice !== ''): ?><p class="message"><?= htmlspecialchars($notice, ENT_QUOTES, 'UTF-8') ?></p><?php endif; ?>
  <?php if ($error !== ''): ?><p class="message error"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></p><?php endif; ?>
  <?php if ($paymentState === 'cancelled'): ?><p class="message error" data-so="Lacag-bixinta waa la joojiyey. Wax lacag ah lama qaadin." data-no="Betalingen ble avbrutt. Ingen penger er trukket.">Lacag-bixinta waa la joojiyey. Wax lacag ah lama qaadin.</p><?php endif; ?>
  <?php if ($paymentState === 'error'): ?><p class="message error" data-so="Lacag-bixinta lama bilaabi karin hadda. Fadlan mar kale isku day." data-no="Betalingen kunne ikke startes nå. Prøv igjen.">Lacag-bixinta lama bilaabi karin hadda. Fadlan mar kale isku day.</p><?php endif; ?>
  <p class="terms" data-so="Stripe ayaa si ammaan ah u maamusha lacag-bixinta. Carruurtu waxay ku xirmaan QR-ka qoyska kadib marka waalidku galo." data-no="Stripe håndterer betalingen sikkert. Barn kobles til med familie-QR etter at forelderen har logget inn.">Stripe ayaa si ammaan ah u maamusha lacag-bixinta. Carruurtu waxay ku xirmaan QR-ka qoyska kadib marka waalidku galo.</p>
</main>
<script>const langButton=document.getElementById('lang');let lang='so';langButton.onclick=()=>{lang=lang==='so'?'no':'so';document.documentElement.lang=lang;langButton.textContent=lang==='so'?'🇳🇴':'🇸🇴';langButton.setAttribute('aria-label',lang==='so'?'Vis norsk tekst':'Muuji Af-Soomaali');document.querySelectorAll('[data-so]').forEach(el=>el.innerHTML=el.dataset[lang])};document.getElementById('showReset').onclick=()=>{document.getElementById('resetBox').classList.toggle('open');document.getElementById('resetEmail').focus()};</script>
</html>
