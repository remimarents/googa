...
const rewardConfig = window.GOOGA_ORDREISE_REWARDS || { catalog: [] };
const rewardCatalog = Array.isArray(rewardConfig.catalog) ? rewardConfig.catalog : [];
...
const state = { level: Math.min(saved.level || 0, Math.max(0, playableLevelCount - 1)), found: saved.found || {}, rewards: saved.rewards || {}, collection: Array.isArray(saved.collection) ? saved.collection : [], letters: [], selectedTiles: new Set(), bonus: [], norwegian: false, collectionOpen: false };
...
function save() { localStorage.setItem(storageKey, JSON.stringify({ level: state.level, found: state.found, rewards: state.rewards, collection: state.collection })); }
...
function rewardFor(id) { return rewardCatalog.find(reward => reward.id === id) || null; }
function rewardForLevel(levelId) { if (!rewardCatalog.length) return null; const uncollected = rewardCatalog.filter(reward => !state.collection.includes(reward.id)); const choices = uncollected.length ? uncollected : rewardCatalog; const reward = choices[Math.floor(Math.random() * choices.length)]; state.rewards[levelId] = reward.id; if (!state.collection.includes(reward.id)) state.collection.push(reward.id); save(); return reward; }
function collectionMarkup() { if (!state.collectionOpen) return ''; const items = rewardCatalog.filter(reward => state.collection.includes(reward.id)); return `<section class="collection-card"><div><p>${text('URURINTAYDA', 'SAMLINGEN MIN')}</p><h2>${text('Waxyaabahaaga', 'Tingene dine')}</h2></div><button type="button" data-action="collection" aria-label="Lukk">×</button><div class="collection-items">${items.length ? items.map(reward => `<span title="${text(reward.so, reward.no)}">${reward.icon}</span>`).join('') : `<small>${text('Dhammayso heer si aad u hesho wax yar.', 'Fullfør et brett for å få en liten ting.')}</small>`}</div><small class="collection-note">${text('Wax walba waa hadiyad ciyaarta ah. Lacag ma aha.', 'Alt er en gave i spillet. Det er ikke penger.')}</small></section>`; }

function render() {
  const level = current(); if (!level) return;
  const found = new Set(state.found[level.id] || []); const complete = level.words.every(word => found.has(word)); if (complete && !state.rewards[level.id]) rewardForLevel(level.id);
  const levelReward = rewardFor(state.rewards[level.id]);
  ...
  app.innerHTML = `<section class="game-shell"><div class="scenery" aria-hidden="true"></div>
    ...
    <section class="board-panel"><div class="progress-row"><span>${found.size}/${level.words.length} ${text('eray', 'ord')}</span><span>${state.bonus.length} ${text('dheeraad', 'bonus')}</span></div><div class="word-board">${level.words.map(word => `<div class="word-slot ${found.has(word) ? 'found' : ''}">${found.has(word) ? word : '•'.repeat(word.length)}</div>`).join('')}</div><div class="message" id="message">${complete ? text('Waa la dhammeeyey!', 'Nivået er løst!') : text('Raadi dhammaan erayada', 'Finn alle ordene')}</div>${complete && levelReward ? `<section class="reward-card"><span>${levelReward.icon}</span><div><p>${text('HADIYAD YAR', 'LITEN GAVE')}</p><strong>${text('Waxaad heshay:', 'Du fikk:')} ${text(levelReward.so, levelReward.no)}</strong><small>${text('Qof walba wax ayuu helaa marka uu heer dhammaysto.', 'Alle får noe når de fullfører et brett.')}</small></div><button type="button" data-action="collection">${text('Ururin', 'Samling')} · ${state.collection.length}</button></section>` : ''}${collectionMarkup()}</section>
